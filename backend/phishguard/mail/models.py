...
class EmailBody:
    ...
    def get_content(self) -> str:
        """Get extracted visible content, falling back to the original body variants."""
...
@dataclass
class EmailLink:
    """A destination and its visible label and nearby body text.

    Attributes:
        url: HTTP(S) destination, without fetching or following it.
        text: Visible anchor label, including image alternative text.
        context: Bounded text from the surrounding paragraph or container.
    """
...
class EmailMessage:
    ...
    link_details: List[EmailLink] = field(default_factory=list)
